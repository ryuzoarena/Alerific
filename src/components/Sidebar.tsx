...
export function Sidebar({ activeView, onViewChange, selectedPlaylistId, onSelectPlaylist }: SidebarProps) {
  const { playlists, songs, createPlaylist, playerState, playSong, isSidebarOpen, closeSidebar } = useMusicStore();
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={cn(
        "h-full bg-black flex flex-col gap-2 p-2 z-50 transition-transform duration-300",
        ...
        <button
          onClick={closeSidebar}
          className="lg:hidden absolute top-4 right-4 p-2 rounded-full hover:bg-accent text-muted-foreground hover:text-foreground transition-colors"
        >
          <X size={20} />
        </button>
